...
class ModelConfig:
    ...
    def __post_init__(self):
        ...
        if self.reference_adapter_path is None:
            assert self.lora_config is not None, "lora_config must be provided when reference_adapter_path is None"
        elif os.path.isdir(self.reference_adapter_path):
            if self.reference_subfolder:
                raise ValueError("Do not use subfolder for local paths, just append to the path instead")
        if self.reference_adapter_path is not None and self.lora_config is not None:
            raise ValueError("lora_config provided along with reference_adapter_path. Only one of the two should be provided.")
...
class VLLMConfig:
    ...
    Args:
        ...
        server_wait_timeout_seconds (int): Time to wait for the vLLM server to become ready at the start of training, in seconds.
        temperature (float): Sampling temperature for generation. Currently only 1.0 is supported except for debugging, because
            the loss calculation does not take into account the temperature.
        correlation_check_threshold (float): Threshold for correlation check between VLLM and HF model logprobs.
        correlation_check_severity (str): Severity level ("suppress", "warning", or "error") for correlation check failures.
...
# Names for KL estimators taken from https://arxiv.org/abs/2506.09477
